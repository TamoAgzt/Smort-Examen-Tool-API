...
public class ExamenPlanner : ControllerBase
{
    ...
    public List<ExamenWeekItem>? GetExamesForAweek()
    {
        ...
        using (DatabaseHandler database = new())
        {
            ...
            SelectAgendaItem.Parameters.AddWithValue("@TimeStartWeek", DateTime.Now.AddDays(-2));
            SelectAgendaItem.Parameters.AddWithValue("TimeEndWeek", DateTime.Now.AddDays(5));

            string DataAgendaItem = database.Select(SelectAgendaItem);
            _logger.Log(LogLevel.Information, DataAgendaItem);
            ...
            foreach (AgendaItem Item in agendaItemExamen)
            {
                ...
                SelectToezichthouder.CommandText = "SELECT Naam, Achternaam FROM Toezichthouders WHERE Id=@Id;";
    ...
    public List<ExamenWeekItem>? GetExamesForAMonth()
    {
        ...
        using (DatabaseHandler database = new())
        {
            ...
            if (KlasId != 0)
            {
                ...
                SelectAgendaItem.Parameters.AddWithValue("@klasId", 0);
            }
            else
                SelectAgendaItem.CommandText = "SELECT Klas_Id, Examen_Id, Lokaal_Id, Tijd_Begin, Tijd_Einden FROM AgendaItem WHERE Tijd_Begin >= @TimeStartWeek AND Tijd_Einden <= @TimeEndWeek;";
            

            SelectAgendaItem.Parameters.AddWithValue("@TimeStartWeek", DateTime.Now.AddDays(-9));
            SelectAgendaItem.Parameters.AddWithValue("TimeEndWeek", DateTime.Now.AddMonths(1));
